...
import { ApiKeyNotProvidedError, CheckoutSessionApiError, InvalidCheckoutSessionInputError, UnauthorizedError } from './error';
...
export default class CheckoutSession implements ICheckoutSession {
    ...
    private verifyCheckoutSessionInput(checkoutSessionInput: ICheckoutSessionInput): void {
        if(!isValidAddress(checkoutSessionInput['serviceAddress'])) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `serviceAddress` field.");
        }

        if(checkoutSessionInput['packageRegistryId'] <= 0) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `packageRegistryId` field.");
        }

        if(!isValidChainId(checkoutSessionInput.chainId)) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `chainId` field.");
        }

        if(!isValidUrl(checkoutSessionInput.cancelUrl)) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `cancelUrl` field.");
        }

        if(!isValidUrl(checkoutSessionInput.successUrl)) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `successUrl` field.");
        }

        const expiresAt = checkoutSessionInput['expiresAt'];

        if(expiresAt && !isValidExpiresAt(expiresAt)) {
            throw new InvalidCheckoutSessionInputError("Invalid checkout session input `expiresAt` field.");
        }
    }
