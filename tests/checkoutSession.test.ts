    ...
    describe("create tests", () => {
        ...
        it("UnauthorizedError should be thrown is the request is unauthorized", async () => {
            ...
            fetchMock.mockResolvedValue({
              ...
              await expect(createCheckoutSessionFn).rejects.toThrow(UnauthorizedError);
              expect(fetchMock).toBeCalledTimes(1);
              await expect(createCheckoutSessionFn).rejects.toThrow("Unathorized request to create checkout session.");
        ...
        it("CheckoutSessionApiError should be thrown is API error is thrown", async () => {
            ...
            fetchMock.mockResolvedValue({
              ...
              await expect(createCheckoutSessionFn).rejects.toThrow(CheckoutSessionApiError);
              expect(fetchMock).toBeCalledTimes(1);
              await expect(createCheckoutSessionFn).rejects.toThrow("API error while creating checkout session: SERVICE_NOT_FOUND");
              expect(fetchMock).toBeCalledTimes(2);
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid serviceAddress is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn).rejects.toThrow("Invalid checkout session input `serviceAddress` field.");
        });
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid packageRegistryId is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn).rejects.toThrow("Invalid checkout session input `packageRegistryId` field.");
        });
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid chainId is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn).rejects.toThrow("Invalid checkout session input `chainId` field.");
        });
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid cancelUrl is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn).rejects.toThrow("Invalid checkout session input `cancelUrl` field.");
        });
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid successUrl is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn).rejects.toThrow("Invalid checkout session input `successUrl` field.");
        });
        ...
        it("InvalidCheckoutSessionInputError should be thrown if invalid expiresAt date is supplied to the checkoutSessionInput", async () => {
            ...
            await expect(createCheckoutSessionFn1).rejects.toThrow(InvalidCheckoutSessionInputError);
            await expect(createCheckoutSessionFn1).rejects.toThrow("Invalid checkout session input `expiresAt` field.");


            expiresAt = new Date(dateTimeNow + 25 * 60 * 60 * 1000);
            let checkoutSessionInput2 = {...checkoutSessionInput, expiresAt: expiresAt}
